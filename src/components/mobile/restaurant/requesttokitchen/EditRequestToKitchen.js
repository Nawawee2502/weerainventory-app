...
import { Br_rtkAlljoindt, updateBr_rtk } from '../../../../api/restaurant/br_rtkApi';
...
export default function EditRequestToKitchen({ onBack, editRefno }) {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                // Load main kitchen request data
                if (editRefno) {
                    const orderResponse = await dispatch(Br_rtkAlljoindt({ refno: editRefno })).unwrap();
    ...
    const handleUpdate = async () => {
        ...
        try {
            Swal.fire({
                title: 'Updating kitchen request...',
            ...
            await dispatch(updateBr_rtk(orderData)).unwrap();
            ...
            <Box display="flex" p={2} bgcolor="#F9F9F9">
                ...
                <Box flex={2} pl={2} bgcolor="#FFF" p={1} borderRadius="12px" boxShadow={3}>
                    ...
                    <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                        <Typography variant="h6" color="#754C27">Edit Request</Typography>
                        <Box>
