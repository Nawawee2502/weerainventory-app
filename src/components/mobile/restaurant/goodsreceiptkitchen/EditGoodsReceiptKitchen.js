    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                if (editRefno) {
                    // ใช้ API getRfkByRefno เพื่อดึงข้อมูลเฉพาะรายการที่ต้องการแก้ไข
                ...
                Swal.fire({
                    icon: 'error',
                    title: 'Error',
                    text: 'Failed to load kitchen receipt data'
                });
            } finally {
                setIsLoading(false);
            }
    ...
    const handleUpdate = async () => {
        if (!saveBranch || !saveKitchen || products.length === 0) {
            Swal.fire({
                ...
                text: 'Please select a branch, kitchen and add at least one product.',
                timer: 1500
            });
            return;
        }
        ...
        try {
            ...
            await Swal.fire({
                icon: 'success',
                title: 'Updated kitchen receipt successfully',
            ...
            Swal.fire({
                icon: 'error',
                title: 'Error',
                text: error.message || 'Error updating kitchen receipt',
    ...
    if (isLoading) {
        return (
            <Box sx={{
                ...
                <Typography variant="h6">Loading kitchen receipt data...</Typography>
