...
export default function KitchenRequisition({ onCreate, onEdit }) {
        ...
        try {
            await Swal.fire({
                ...
                if (result.isConfirmed) {
                    await dispatch(deleteKt_grf({ refno })).unwrap();
                    Swal.fire(
                        'Deleted!',
    ...
    const handleDeleteSelected = async () => {
        try {
            await Swal.fire({
                ...
                if (result.isConfirmed) {
                    await Promise.all(
                        selected.map(refno => dispatch(deleteKt_grf({ refno })).unwrap())
                    );
                    Swal.fire(
                        'Deleted!',
