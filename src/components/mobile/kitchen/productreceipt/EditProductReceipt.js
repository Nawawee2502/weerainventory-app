...
import { Kt_prfByRefno, updateKt_prf } from '../../../../api/kitchen/kt_prfApi';
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                // Fetch receipt data
                const receiptResponse = await dispatch(Kt_prfByRefno({ refno: editRefno })).unwrap();
                if (receiptResponse.result && receiptResponse.data) {
                    ...
                    if (receiptData.kt_prfdts && receiptData.kt_prfdts.length > 0) {
                        const productsData = [];
                        ...
                        receiptData.kt_prfdts.forEach(item => {
        ...
        <Box sx={{ width: '100%', p: 2 }}>
            ...
            <Box sx={{ backgroundColor: '#fff', borderRadius: '10px', p: 3, boxShadow: '0px 4px 10px rgba(0, 0, 0, 0.1)' }}>
                <Typography variant="h6" sx={{ color: '#754C27', mb: 3 }}>
                    Edit Product Receipt: {editRefno}
                </Typography>
