...
export default function CreateTransferToWarehouse({ onBack }) {
    ...
    const handleGetLastRefNo = async (selectedDate) => {
        try {
            ...
            const res = await dispatch(Kt_trwrefno({ month, year })).unwrap();

            // If no reference numbers exist yet, start with 001
            if (!res.data || !res.data.refno) {
                const prefix = `KTTRW${year}${month}`;
            ...
            setLastRefNo(`KTTRW${year}${month}${timestamp}`);
        } finally {
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            // Submit the data
            await dispatch(addKt_trw(orderData)).unwrap();
