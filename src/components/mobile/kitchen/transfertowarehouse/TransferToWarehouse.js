...
import { Kt_trwAlljoindt, deleteKt_trw, Kt_trwByRefno } from '../../../../api/kitchen/kt_trwApi';
import { kitchenAll } from '../../../../api/kitchenApi';
import { searchProductName } from '../../../../api/productrecordApi';
import Swal from 'sweetalert2';
import { pdf } from '@react-pdf/renderer';
import { generatePDF } from './Kt_trwPDF';
...
export default function TransferToWarehouse({ onCreate, onEdit }) {
        ...
        try {
            await Swal.fire({
                ...
                if (result.isConfirmed) {
                    await dispatch(deleteKt_trw({ refno })).unwrap();
                    Swal.fire(
                        'Deleted!',
    ...
    const handleDeleteSelected = async () => {
        try {
            await Swal.fire({
                ...
                if (result.isConfirmed) {
                    await Promise.all(
                        selected.map(refno => dispatch(deleteKt_trw({ refno })).unwrap())
                    );
                    Swal.fire(
                        'Deleted!',
    ...
    const handlePrintPDF = async (refno) => {
        try {
            ...
            // ดึงข้อมูลจาก API
            const response = await dispatch(Kt_trwByRefno({ refno })).unwrap();
