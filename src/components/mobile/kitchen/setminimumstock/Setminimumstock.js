...
import { addKtMinStock, queryKtMinStock, deleteKtMinStock, updateKtMinStock, checkMinStockExists } from '../../../../api/kitchen/kt_minimum_stockApi';
...
export default function KitchenSetMinimumStock() {
    ...
    const formik = useFormik({
        ...
        validate: values => {
            ...
            if (!values.kitchen_code) {
                errors.kitchen_code = 'Kitchen is required';
            }
        ...
        onSubmit: async (values) => {
            try {
                ...
                if (!isEditing) {
                    ...
                    if (checkResult.exists) {
                        Swal.fire({
                            ...
                            text: 'This product is already set for this kitchen'
                        });
                        return;
                    }
                }
                ...
                if (isEditing) {
                    await dispatch(updateKtMinStock(minStockData)).unwrap();
                    Swal.fire({
                        icon: 'success',
                    ...
                    await dispatch(addKtMinStock(minStockData)).unwrap();
                    Swal.fire({
                        icon: 'success',
    ...
    const refreshData = () => {
        ...
        // เพิ่ม kitchen_code ถ้ามีการเลือก
        if (selectedKitchen) {
        ...
        // เรียก API
        dispatch(queryKtMinStock(params))
            .unwrap()
            .then((res) => {
    ...
    useEffect(() => {
        refreshData();
    }, [page, selectedKitchen, tableSearchTerm]);

    // โหลด units และ kitchens
    useEffect(() => {
        let offset = 0;
        let limit = 100;
    ...
    // จัดการการเปลี่ยนครัวในตัวกรอง
    const handleKitchenChange = (e) => {
        const kitchenCode = e.target.value;
        setSelectedKitchen(kitchenCode);
        setPage(1);
    };

    // จัดการการเปลี่ยนครัวในฟอร์ม
    const handleFormKitchenChange = (e) => {
        const kitchenCode = e.target.value;
        formik.setFieldValue('kitchen_code', kitchenCode);

        // อัพเดทชื่อครัวในฟอร์ม
        updateKitchenName(kitchenCode);
    };

    // Helper function สำหรับอัพเดทชื่อครัวตาม kitchen code
    const updateKitchenName = (kitchenCode) => {
        if (kitchenCode) {
            const selectedKitchenData = kitchens.find(kitchen => kitchen.kitchen_code === kitchenCode);
            if (selectedKitchenData) {
                formik.setFieldValue('kitchen', selectedKitchenData.kitchen_name || '');
            }
        } else {
    ...
    const handleEdit = (stock) => {
        ...
        // Find kitchen details to get kitchen name
        const kitchenData = kitchens.find(kitchen => kitchen.kitchen_code === stock.kitchen_code);
        const kitchenName = kitchenData ? kitchenData.kitchen_name : '';
    ...
    // จัดการการลบ
    const handleDelete = (product_code, kitchen_code) => {
        Swal.fire({
            title: 'Are you sure?',
            ...
            if (result.isConfirmed) {
                dispatch(deleteKtMinStock({ product_code, kitchen_code }))
                    .unwrap()
                    .then(() => {
                        Swal.fire(
                            'Deleted!',
    ...
    // ดึงชื่อครัวจาก kitchen code
    const getKitchenName = (kitchenCode) => {
        if (!kitchenCode) return '';
        const kitchen = kitchens.find(k => k.kitchen_code === kitchenCode);
                        ...
                        <TableBody>
                            {minStocks.length === 0 ? (
                                ...
                                minStocks.map((stock, index) => (
                                    <StyledTableRow key={`${stock.product_code}-${stock.kitchen_code}`}>
                                        ...
                                        <StyledTableCell align="center">
                                            <Box sx={{ display: 'flex', justifyContent: 'center', gap: 1 }}>
                                                ...
                                                <IconButton
                                                    onClick={() => handleDelete(stock.product_code, stock.kitchen_code)}
                            ...
                            <Box sx={{ mb: 2 }}>
                                ...
                                <FormControl fullWidth>
                                    <Select
                                        ...
                                        onChange={handleFormKitchenChange}
                                        error={formik.touched.kitchen_code && Boolean(formik.errors.kitchen_code)}
                                        displayEmpty
                                        sx={{
                                            borderRadius: '10px',
                                        }}
                                    >
                                        <MenuItem value="">Select Kitchen</MenuItem>
                                        {kitchens.map((kitchen) => (
                                            <MenuItem key={kitchen.kitchen_code} value={kitchen.kitchen_code}>
                                                {kitchen.kitchen_name}
                                            </MenuItem>
