...
const PurchaseOrderToWarehousePDF = ({ data, excludePrice = false, startDate, endDate }) => {
    ...
    headers.push({ title: 'Unit', style: styles.cellUnit });
...
export const exportToPdfPow = async (data, excludePrice = false, startDate, endDate) => {
    try {
        const blob = await pdf(
            <PurchaseOrderToWarehousePDF
                data={data}
        ...
        link.href = url;
        link.download = `purchase_order_to_warehouse_${new Date().toISOString().split('T')[0]}.pdf`;
