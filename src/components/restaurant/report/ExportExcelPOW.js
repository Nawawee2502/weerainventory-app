...
export const exportToExcelPow = async (data, excludePrice = false, startDate, endDate) => {
    const workbook = new ExcelJS.Workbook();
    const worksheet = workbook.addWorksheet('Purchase Order to Warehouse', {
        views: [{
            showGridLines: false
        }],
        pageSetup: {
    ...
    const headerRows = [
        ...
        {
            text: 'Purchase Order to Warehouse',
    ...
    const fileName = `purchase_order_to_warehouse_${new Date().toISOString().split('T')[0]}.xlsx`;
