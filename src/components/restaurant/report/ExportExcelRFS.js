...
export const exportToExcelRfs = async (data, excludePrice = false, startDate, endDate) => {
    const workbook = new ExcelJS.Workbook();
    const worksheet = workbook.addWorksheet('Receipt From Supplier (Branch)', {
        views: [{
            showGridLines: false
        }],
        pageSetup: {
    ...
    const headerRows = [
        ...
        {
            text: 'Receipt From Supplier (Branch)',
    ...
    const fileName = `receipt_from_supplier_branch_${new Date().toISOString().split('T')[0]}.xlsx`;
