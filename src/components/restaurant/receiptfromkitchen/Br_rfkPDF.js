...
// Define styles for restaurant goods receipt format
...
// Main PDF Component for Goods Receipt from Kitchen
export const KitchenGoodsReceiptPDF = ({ refno, date, branch, branchName, kitchen, kitchenName, productArray, total, username, data, branchAddr1, branchAddr2, branchTel1, includePrices = true }) => {
    return (
        <Document>
            <Page style={styles.page} size="A4">
                ...
                {/* Document Title */}
                <Text style={styles.poTitle}>GOODS RECEIPT FROM KITCHEN</Text>

                {/* Restaurant & Kitchen Info */}
                ...
                <View style={styles.signatureSection}>
                    ...
                    <View style={styles.signatureBox}>
                        <Text style={styles.signatureLine}></Text>
                        <Text style={styles.signatureText}>Restaurant Manager</Text>
                    </View>
                    <View style={styles.signatureBox}>
                        <Text style={styles.signatureLine}></Text>
...
export const generateKitchenGoodsReceiptPDF = async (refno, data, includePrices = true) => {
    if (!data) return null;

    console.log("Kitchen Goods Receipt PDF Generation - Data received:", data);
    console.log("Kitchen Goods Receipt PDF Generation - Include prices:", includePrices);
    console.log("BR_RFKDTS data:", data.br_rfkdts);

    if (data.br_rfkdts) {
        console.log("BR_RFKDTS type:", typeof data.br_rfkdts);
        console.log("BR_RFKDTS is array:", Array.isArray(data.br_rfkdts));
        console.log("BR_RFKDTS length:", data.br_rfkdts?.length);

        // If br_rfkdts is an object but not an array, let's convert it to an array
        if (typeof data.br_rfkdts === 'object' && !Array.isArray(data.br_rfkdts)) {
            console.log("Converting br_rfkdts object to array");
            data.br_rfkdts = Object.values(data.br_rfkdts);
            console.log("After conversion, BR_RFKDTS length:", data.br_rfkdts?.length);
        }
    }
    ...
    // Use enhanced logging and error handling for br_rfkdts
    try {
        if (data.br_rfkdts && Array.isArray(data.br_rfkdts)) {
            console.log("Processing br_rfkdts array with length:", data.br_rfkdts.length);

            productItems = data.br_rfkdts.map((item, index) => {
            ...
            console.log("BR_RFKDTS is not available or not an array");
        }
    } catch (error) {
        console.error("Error processing br_rfkdts:", error);
        productItems = [];
    }

    const pdfContent = (
        <KitchenGoodsReceiptPDF
            refno={data.refno || ''}
            date={data.rdate || ''}
