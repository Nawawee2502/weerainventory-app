            ...
            try {
                ...
                if (editRefno) {
                    ...
                    if (receiptResponse && receiptResponse.result && receiptResponse.data) {
                        ...
                        // Get taxable and non-taxable amounts
                        const initialTaxable = parseFloat(headerData.taxable) || 0;
                        const initialNonTaxable = parseFloat(headerData.nontaxable) || 0;
                        const initialTotal = parseFloat(headerData.total) || 0;

                        setTaxableAmount(initialTaxable);
                        setNonTaxableAmount(initialNonTaxable);
                        setTotal(initialTotal);
                        setSaleTax(initialTaxable * TAX_RATE);
                        setTotalDue(initialTotal + (initialTaxable * TAX_RATE));
                                        ...
                                        {products.length === 0 ? (
                                            <tr>
                                                <td colSpan={11} style={{ padding: '20px', textAlign: 'center' }}>
                                                    No products found or failed to load product data. Try searching for products to add.
                                                </td>
                                            </tr>
                                        ) : (
