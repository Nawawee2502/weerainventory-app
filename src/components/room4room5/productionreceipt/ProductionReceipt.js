...
import { Kt_prfAlljoindt, deleteKt_prf, Kt_prfByRefno } from '../../../api/kitchen/kt_prfApi';
import { kitchenAll } from '../../../api/kitchenApi';
import { searchProductName } from '../../../api/productrecordApi';
import Swal from 'sweetalert2';
import { pdf } from '@react-pdf/renderer';
import { generatePDF } from './Kt_prfPDF';
...
export default function ProductionReceipt({ onCreate, onEdit }) {
        ...
        try {
            await Swal.fire({
                ...
                if (result.isConfirmed) {
                    await dispatch(deleteKt_prf({ refno })).unwrap();
                    Swal.fire(
                        'Deleted!',
    ...
    const handleDeleteSelected = async () => {
        try {
            await Swal.fire({
                ...
                if (result.isConfirmed) {
                    await Promise.all(
                        selected.map(refno => dispatch(deleteKt_prf({ refno })).unwrap())
                    );
                    Swal.fire(
                        'Deleted!',
    ...
    const handlePrintPDF = async (refno) => {
        try {
            ...
            // ดึงข้อมูลจาก API
            const response = await dispatch(Kt_prfByRefno({ refno })).unwrap();
