...
import { Kt_rfsAlljoindt, deleteKt_rfs, Kt_rfsByRefno } from '../../../api/kitchen/kt_rfsApi';
import { supplierAll } from '../../../api/supplierApi';
import { searchProductName } from '../../../api/productrecordApi';
import Swal from 'sweetalert2';
import { pdf } from '@react-pdf/renderer';
import { generatePDF } from './Kt_rfsPDF';
...
export default function ReceiptFromSupplier({ onCreate, onEdit }) {
    ...
    const handlePrintPDF = async (refno) => {
        try {
            ...
            // ดึงข้อมูลจาก API
            const response = await dispatch(Kt_rfsByRefno({ refno })).unwrap();
