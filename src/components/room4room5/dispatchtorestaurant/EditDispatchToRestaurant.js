...
import { updateKt_dpb, getKtDpbByRefno } from '../../../api/kitchen/kt_dpbApi';
...
export default function EditDispatchToRestaurant({ onBack, editRefno }) {
    ...
    const loadDispatchData = async () => {
        try {
            console.log('Loading dispatch data for:', editRefno);

            // Get dispatch header data
            const headerResponse = await dispatch(getKtDpbByRefno(
                typeof editRefno === 'object' ? editRefno : { refno: editRefno }
            )).unwrap();

            if (headerResponse.result && headerResponse.data) {
                const dispatchData = headerResponse.data;
                setLastRefNo(dispatchData.refno || '');
                ...
                setSaveKitchen(dispatchData.kitchen_code || '');
                setSaveBranch(dispatchData.branch_code || '');

                // Set totals
                setTotal(parseFloat(dispatchData.total) || 0);
                setTaxableAmount(parseFloat(dispatchData.taxable) || 0);
                setNonTaxableAmount(parseFloat(dispatchData.nontaxable) || 0);
                setTotalDue((parseFloat(dispatchData.total) || 0) +
                    ((parseFloat(dispatchData.taxable) || 0) * TAX_RATE));
            }

            // Get dispatch detail data
            const detailResponse = await dispatch(Kt_dpbdtAlljoindt(
                typeof editRefno === 'object' ? editRefno : { refno: editRefno }
            )).unwrap();
    ...
    const processDetailData = async (detailData) => {
        try {
            ...
            // Calculate totals
            calculateOrderTotals(productsData, newQuantities, newUnitPrices, newTaxStatus);
    ...
    const handleProductSelect = (product) => {
        ...
        const newTaxStatus = { ...taxStatus, [product.product_code]: product.tax1 || "N" };

        calculateOrderTotals(newProducts, newQuantities, newUnitPrices, newTaxStatus);
    };
    ...
    const calculateOrderTotals = (currentProducts = products, currentQuantities = quantities, currentUnitPrices = unitPrices, currentTaxStatus = taxStatus) => {
        let newTotals = {};
        let newTotal = 0;
        let newTaxable = 0;
        let newNonTaxable = 0;
        ...
        currentProducts.forEach(product => {
            ...
            if (currentTaxStatus[productCode] === 'Y' || product.tax1 === 'Y') {
    ...
    const handleUnitChange = (productCode, newUnitCode) => {
        ...
        calculateOrderTotals(products, quantities, newUnitPrices, taxStatus);
    };
    ...
    const handleQuantityInputChange = (productCode, value) => {
        ...
        calculateOrderTotals(products, newQuantities, unitPrices, taxStatus);
    };
    ...
    const handleTaxStatusChange = (productCode, newStatus) => {
        ...
        // Set the updated tax status
        setTaxStatus(newTaxStatus);

        // Calculate totals with the new tax status directly
        calculateOrderTotals(products, quantities, unitPrices, newTaxStatus);
    };
    ...
    const handleUpdate = async () => {
        ...
        try {
            ...
            // Prepare header data - ensure refno is a string value
            const headerData = {
                refno: lastRefNo,
            ...
            // Prepare footer data with string values to avoid type issues
            const footerData = {
            ...
            // Submit the data
            const response = await dispatch(updateKt_dpb(orderData)).unwrap();
            console.log("API response:", response);

            // Show success message
            await Swal.fire({
                icon: 'success',
                title: 'Updated dispatch successfully',
            ...
            Swal.fire({
                icon: 'error',
                title: 'Error',
                text: error?.message || 'Error updating dispatch',
