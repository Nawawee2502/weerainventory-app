...
import { kt_powAlljoindt } from '../../../api/kitchen/kt_powApi';
import { exportToExcelWhPow } from './ExportExcelPurchaseOrderToWarehouse';
import { branchAll } from '../../../api/branchApi';
import { exportToPdfWhPow } from './ExportPdfPurchaseOrderToWarehouse';
import PrintLayout from './PrintPreviewPurchaseOrderToWarehouse';
import Swal from 'sweetalert2';
import { createRoot } from 'react-dom/client';  // <-- เพิ่มบรรทัดนี้


export default function ReportPurchaseOrderToWarehouse() {
    const today = new Date();
    const [whpowData, setWhpowData] = useState([]);
    ...
    const fetchData = async (params) => {
        try {
            const response = await dispatch(kt_powAlljoindt(params)).unwrap();
            console.log("API Response:", response);

            if (response.data) {
                const flattenedData = response.data.flatMap(order =>
                    order.wh_powdts.map(detail => ({
                        date: order.rdate,
                        refno: order.refno,
    ...
    const handleSearch = (productSearch = searchProduct) => {
        ...
        dispatch(kt_powAlljoindt(params))
            .unwrap()
            .then(res => {
                if (!res.data) {
    ...
    const handleExportExcel = () => {
        ...
        exportToExcelWhPow(whpowData, excludePrice, startDate, endDate);
    };
    ...
    const handleExportPdf = () => {
        ...
        exportToPdfWhPow(whpowData, excludePrice, startDate, endDate);
    };
