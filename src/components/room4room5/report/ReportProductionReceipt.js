...
import { Kt_prfAlljoindt } from '../../../api/kitchen/kt_prfApi';
import { exportToExcelPrf } from './ExportExcelPRF';
import { kitchenAll } from '../../../api/kitchenApi';
import { exportToPdfPrf } from './ExportPdfPRF';
import PrintLayout from './PrintPreviewPRF';
import Swal from 'sweetalert2';

export default function ReportProductionReceipt() {
    ...
    const fetchData = async (params) => {
        try {
            const response = await dispatch(Kt_prfAlljoindt(params)).unwrap();
            if (response.data) {
                const flattenedData = response.data.flatMap(order =>
                    order.kt_prfdts.map(detail => ({
                        date: order.rdate,
                        refno: order.refno,
    ...
    const handleExportExcel = () => {
        ...
        exportToExcelPrf(prfData, excludePrice, startDate, endDate);
    };
    ...
    const handleExportPdf = () => {
        ...
        exportToPdfPrf(prfData, excludePrice, startDate, endDate);
    };
