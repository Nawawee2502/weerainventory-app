...
import { Kt_trwAlljoindt } from '../../../api/kitchen/kt_trwApi';
import { exportToExcelTrw } from './ExportExcelTRW';
import { branchAll } from '../../../api/branchApi';
import { kitchenAll } from '../../../api/kitchenApi';
import { exportToPdfTrw } from './ExportPdfTRW';
import PrintLayout from './PrintPreviewTRW';
import Swal from 'sweetalert2';

export default function ReportTransferToWarehouse() {
    ...
    const fetchData = async (params) => {
        try {
            const response = await dispatch(Kt_trwAlljoindt(params)).unwrap();
            console.log("API Response:", response);

            if (response.data) {
                const flattenedData = response.data.flatMap(order =>
                    order.kt_trwdts.map(detail => ({
                        date: order.rdate,
                        refno: order.refno,
    ...
    const handleExportExcel = () => {
        ...
        exportToExcelTrw(trwData, excludePrice, startDate, endDate);
    };
    ...
    const handleExportPdf = () => {
        ...
        exportToPdfTrw(trwData, excludePrice, startDate, endDate);
    };
