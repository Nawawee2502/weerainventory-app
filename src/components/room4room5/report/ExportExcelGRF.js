...
export const exportToExcelGrf = async (data, excludePrice = false, startDate, endDate) => {
    ...
    const headers = [
        'No.', 'Date', 'Ref.no', 'Kitchen', 'Product ID',
    ...
    const columnWidths = {
        'No.': 8, 'Date': 15, 'Ref.no': 15, 'Kitchen': 25,
    ...
    groupedData.forEach((item, index) => {
        const rowData = [
            index + 1, item.date, item.refno, item.kitchen,
    ...
    const summaryRow = worksheet.addRow(headers.map(header =>
        header === 'Total' && !excludePrice ? totalSum.toFixed(2) : ''
    ));
