...
export const exportToExcelDpb = async (data, excludePrice = false, startDate, endDate) => {
    const workbook = new ExcelJS.Workbook();
    const worksheet = workbook.addWorksheet('Dispatch to Restaurant', {
        views: [{
            showGridLines: false
        }],
        pageSetup: {
    ...
    const fileName = `dispatch_to_restaurant_${new Date().toISOString().split('T')[0]}.xlsx`;
