...
export const exportToExcelTrw = async (data, excludePrice = false, startDate, endDate) => {
    const workbook = new ExcelJS.Workbook();
    const worksheet = workbook.addWorksheet('Transfer to Warehouse', {
        views: [{
            showGridLines: false
        }],
        pageSetup: {
    ...
    const fileName = `transfer_to_warehouse_${new Date().toISOString().split('T')[0]}.xlsx`;
