...
export const exportToExcelPrf = async (data, excludePrice = false, startDate, endDate) => {
    const workbook = new ExcelJS.Workbook();
    const worksheet = workbook.addWorksheet('Production Receipt', {
    ...
    const headerRows = [
        ...
        { text: 'Production Receipt', font: { bold: true, size: 12 } }
    ];
    ...
    if (!excludePrice) {
        ...
        const totalCellColumn = headers.indexOf('Total') + 1;
        const totalRow = worksheet.addRow(new Array(headers.length).fill(''));

        const totalLabelCell = totalRow.getCell(totalCellColumn - 1);
        totalLabelCell.value = 'Total:';
        totalLabelCell.alignment = { horizontal: 'right' };

        const totalValueCell = totalRow.getCell(totalCellColumn);
    ...
    const fileName = `production_receipt_${new Date().toISOString().split('T')[0]}.xlsx`;
