...
import { kt_dpbAlljoindt } from '../../../api/kitchen/kt_dpbApi';
import { exportToExcelDpb } from './ExportExcelDPB';
import { branchAll } from '../../../api/branchApi';
import { kitchenAll } from '../../../api/kitchenApi';
import { exportToPdfDpb } from './ExportPdfDPB';
import PrintLayout from './PrintPreviewDPB';
import Swal from 'sweetalert2';

export default function ReportDispatchToRestaurant() {
    ...
    const [restaurants, setRestaurants] = useState([]); // Will contain branch data
    ...
    const fetchData = async (params) => {
        try {
            const response = await dispatch(kt_dpbAlljoindt(params)).unwrap();
            console.log("API Response:", response);

            if (response.data) {
                const flattenedData = response.data.flatMap(order =>
                    order.kt_dpbdts.map(detail => ({
                        date: order.rdate,
                        refno: order.refno,
                        kitchen: order.tbl_kitchen?.kitchen_name,
                        restaurant: order.tbl_branch?.branch_name,
    ...
    useEffect(() => {
        ...
        // Load restaurant data (using branch API)
    ...
    const handleSearch = (productSearch = searchProduct) => {
        ...
        if (selectedRestaurant) params.branch_code = selectedRestaurant;
    ...
    const handleExportExcel = () => {
        ...
        exportToExcelDpb(dpbData, excludePrice, startDate, endDate);
    };
    ...
    const handleExportPdf = () => {
        ...
        exportToPdfDpb(dpbData, excludePrice, startDate, endDate);
    };
                            ...
                            <Grid2 item size={{ xs: 12, md: 6 }}>
                                ...
                                >
                                    ...
                                    {restaurants.map(restaurant => (
                                        <option key={restaurant.branch_code} value={restaurant.branch_code}>
                    ...
                    <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 2 }}>
                        <Box sx={{ display: 'flex' }}>
                            ...
                            <Box sx={{ ml: '8px' }}>
                                ...
                                <Typography>
                                    {restaurants.find(r => r.branch_code === selectedRestaurant)?.branch_name || "Not selected"}
                                </Typography>
                            </Box>
                        </Box>
