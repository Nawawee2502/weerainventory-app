...
import { wh_rfkAlljoindt } from '../../../api/warehouse/wh_rfkApi';
import { supplierAll } from '../../../api/supplierApi';
import { branchAll } from '../../../api/branchApi';
import { kitchenAll } from '../../../api/kitchenApi'
import Swal from 'sweetalert2';
import { exportToExcelWhRfk } from './ExportExcelReceiptFromKitchen';
import { exportToPdfWhRfk } from './ExportPdfReceiptFromKitchen';
import PrintLayout from './PrintPreviewWhRfk';
import { createRoot } from 'react-dom/client';

export default function ReportReceiptFromKitchen() {
    const today = new Date();
    const [rfkData, setRfkData] = useState([]);
    ...
    const fetchData = async (params) => {
        try {
            ...
            const response = await dispatch(wh_rfkAlljoindt(params)).unwrap();
            ...
            const flattenedData = response.data.flatMap(receipt => {
                if (!receipt.wh_rfkdts || !Array.isArray(receipt.wh_rfkdts)) {
                    console.log("Invalid receipt or missing wh_rfkdts:", receipt);
                    return [];
                }

                return receipt.wh_rfkdts.map(detail => ({
                    date: receipt.rdate,
                    refno: receipt.refno,
                    kitchen_code: receipt.tbl_kitchen?.kitchen_name || 'N/A',
        ...
        const initializeData = async () => {
            ...
            try {
                await Promise.all([
                    ...
                    dispatch(kitchenAll({ offset: 0, limit: 1000 }))  // สมมติว่ามี API kitchenAll
                        .unwrap()
    ...
    const handleExportExcel = () => {
        ...
        exportToExcelWhRfk(rfkData, excludePrice, startDate, endDate);
    };
    ...
    const handleExportPdf = () => {
        ...
        exportToPdfWhRfk(rfkData, excludePrice, startDate, endDate);
    };
                    ...
                    <Box sx={{ width: '90%', mt: '24px' }}>
                        <Grid2 container spacing={2}>
                            ...
                            <Grid2 item size={{ xs: 12, md: 6 }}>
                                ...
                                >
                                    <option value="">Select a Kitchens</option>
