...
const ReceiptFromKitchenPDF = ({ data, excludePrice = false, startDate, endDate }) => {
        ...
        <Document>
            <Page size="A4" orientation="landscape" style={styles.page}>
                <View style={styles.header}>
                    ...
                    <Text style={styles.titleText}>Receipt From Kitchen</Text>
                ...
                {groupedData.map((row, index) => (
                    <View key={index} style={styles.tableRow}>
                        ...
                        <Text style={styles.cellKitchen}>{row.kitchen_code}</Text>
...
export const exportToPdfWhRfk = async (data, excludePrice = false, startDate, endDate) => {
    try {
        const blob = await pdf(
            <ReceiptFromKitchenPDF
                data={data}
        ...
        link.href = url;
        link.download = `receipt_from_kitchen_${new Date().toISOString().split('T')[0]}.pdf`;
