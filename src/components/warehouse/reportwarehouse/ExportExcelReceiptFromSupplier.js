...
export const exportToExcelWhRfs = async (data, excludePrice = false, startDate, endDate) => {
    const workbook = new ExcelJS.Workbook();
    const worksheet = workbook.addWorksheet('Receipt From Supplier', {
        views: [{
            showGridLines: false
        }],
        pageSetup: {
    ...
    const fileName = `receipt_from_supplier_${new Date().toISOString().split('T')[0]}.xlsx`;
