...
export const exportToExcelWhRfk = async (data, excludePrice = false, startDate, endDate) => {
    const workbook = new ExcelJS.Workbook();
    const worksheet = workbook.addWorksheet('Receipt From Kitchen', {
        views: [{
            showGridLines: false
        }],
        pageSetup: {
    ...
    const columnWidths = {
        ...
        'Ref.no': 15,
        'Kitchen': 25,
        'Product Name': 25, // Reduced from 30 to accommodate temperature column
    ...
    const fileName = `receipt_from_kitchen_${new Date().toISOString().split('T')[0]}.xlsx`;
