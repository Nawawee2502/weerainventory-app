...
export const exportToExcelWhDpb = async (data, excludePrice = false, startDate, endDate) => {
    const workbook = new ExcelJS.Workbook();
    const worksheet = workbook.addWorksheet('Dispatch To Branch', {
        views: [{
            showGridLines: false
        }],
        pageSetup: {
    ...
    const fileName = `dispatch_to_branch_${new Date().toISOString().split('T')[0]}.xlsx`;
