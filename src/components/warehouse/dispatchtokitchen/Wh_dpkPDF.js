...
// Define styles for kitchen dispatch format
...
// Main PDF Component for Kitchen Dispatch
export const KitchenDispatchPDF = ({ refno, date, kitchen, kitchenName, productArray, total, username, data, includePrices = true }) => {
    return (
        <Document>
            <Page style={styles.page} size="A4">
                ...
                <View style={styles.header}>
                    <View style={styles.headerLeft}>
                        ...
                        <Text style={styles.companyInfo}>Kitchen Dispatch</Text>
                    </View>
                ...
                {/* Document Title */}
                <Text style={styles.poTitle}>WAREHOUSE DISPATCH TO KITCHEN</Text>
                ...
                <View style={styles.footer}>
                    <Text>This is an official dispatch document for Kitchen. Dispatch# {refno}</Text>
                </View>
            </Page>
        </Document>
    );
...
export const generateKitchenPDF = async (refno, data, includePrices = true) => {
    if (!data) return null;

    console.log("Kitchen PDF Generation - Data received:", data);
    console.log("Kitchen PDF Generation - Include prices:", includePrices);
    console.log("WH_DPKDTS data:", data.wh_dpkdts);

    if (data.wh_dpkdts) {
        console.log("WH_DPKDTS type:", typeof data.wh_dpkdts);
        console.log("WH_DPKDTS is array:", Array.isArray(data.wh_dpkdts));
        console.log("WH_DPKDTS length:", data.wh_dpkdts?.length);

        // If wh_dpkdts is an object but not an array, let's convert it to an array
        if (typeof data.wh_dpkdts === 'object' && !Array.isArray(data.wh_dpkdts)) {
            console.log("Converting wh_dpkdts object to array");
            data.wh_dpkdts = Object.values(data.wh_dpkdts);
            console.log("After conversion, WH_DPKDTS length:", data.wh_dpkdts?.length);
        }
    }
    ...
    // Use enhanced logging and error handling for wh_dpkdts
    try {
        if (data.wh_dpkdts && Array.isArray(data.wh_dpkdts)) {
            console.log("Processing wh_dpkdts array with length:", data.wh_dpkdts.length);

            productItems = data.wh_dpkdts.map((item, index) => {
            ...
            console.log("WH_DPKDTS is not available or not an array");
        }
    } catch (error) {
        console.error("Error processing wh_dpkdts:", error);
        productItems = [];
    }
